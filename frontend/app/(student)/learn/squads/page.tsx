...
function SquadSpotlight({ squad }: { squad: any }) {
  return (
    ...
    >
      <div className="p-5">
        <span className="flex items-center gap-1.5 text-[12.5px] font-bold uppercase tracking-wider text-[#6b0000]">
          <Briefcase size={12} strokeWidth={2} />
          {squad.skillName}
        </span>
        <h2 className="mt-0.5 font-display text-[21.5px] font-bold text-[#201d1d]">
          {squad.name}
        </h2>
        ...
        <div className="mt-3 flex items-center gap-2.5">
          <div className="flex -space-x-2">
            {Array.from({ length: squad.members.total }).map((_, n) => (
              <div
                key={n}
                className="flex h-8 w-8 items-center justify-center rounded-full border-2 border-white bg-[#e0dcd5] text-[12.5px] font-bold text-[#5e5a5a]"
              >
                {String.fromCharCode(65 + n)}
              </div>
            ))}
          </div>
          <span className="flex items-center gap-1 text-[13.5px] font-bold text-emerald-600">
        ...
        <div className="mt-4 flex flex-col gap-1 rounded-[18px] bg-[#fff8ee] p-3.5 border border-[#f59e0b]/15">
          ...
          <p className="text-[14px] font-medium leading-snug text-[#201d1d]">
            {squad.reason}
          </p>
        </div>
        ...
        <div className="mt-4 flex flex-col gap-3 border-t border-black/[0.05] pt-4">
          ...
          {squad.activityFeed?.slice(0, 3).map((activity: any, i: number) => (
            <div key={i} className="flex items-center gap-2.5">
              <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-[#f0ede9] text-[11.5px] font-bold text-[#5e5a5a]">
                {activity.user[0]}
              </div>
              <p className="flex-1 text-[13.5px] leading-snug text-[#5e5a5a]">
              ...
              <span className="shrink-0 text-[12px] text-[#9c9595]">{activity.time}</span>
            </div>
          ))}
        </div>
      </div>
    ...
    <motion.div variants={fadeUp} className="flex flex-col overflow-hidden rounded-[24px] border border-black/[0.05] bg-white shadow-[0_4px_20px_rgba(0,0,0,0.04)]">
      <div className="p-5">
        <div className="flex items-start justify-between">
          <div className="flex flex-col gap-1">
            ...
            <h3 className="font-display text-[19.5px] font-bold text-[#201d1d]">{squad.name}</h3>
          </div>
        </div>

        <div className="mt-3 rounded-lg bg-[#faf9f8] p-3">
          <span className="text-[13px] font-bold text-[#d97706]">{squad.label}</span>
          <p className="mt-1 text-[13px] leading-snug text-[#5e5a5a]">{squad.reason}</p>
        </div>
        ...
        <div className="mt-4 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div className="flex flex-col">
              <span className="text-[12.5px] font-bold text-[#201d1d]">{squad.members.total} members</span>
              <span className="text-[11.5px] text-[#7a7373]">{squad.members.activeThisWeek} active this week</span>
            </div>
          </div>
        </div>
        ...
        <div className="mt-4 flex flex-col gap-2 border-t border-black/[0.05] pt-4">
          <div className="flex flex-col gap-1">
            ...
            <p className="text-[14.5px] font-bold text-[#201d1d]">{squad.outcome}</p>
            <p className="text-[13.5px] text-[#7a7373]">{squad.learningFormat.modules} modules</p>
          </div>
          <div className="mt-2 flex flex-col gap-1">
            <span className="text-[12.5px] font-bold uppercase tracking-wider text-[#7a7373]">
              Weekly Goal
            </span>
            <p className="text-[14.5px] font-bold text-[#201d1d]">{squad.goal.text}</p>
          </div>
        </div>
      ...
      <div className="mt-auto flex flex-col gap-4 bg-[#faf9f8] p-5">
        <div className="flex items-start gap-2">
          ...
          <div className="flex flex-col">
            <span className="text-[12.5px] font-medium text-[#7a7373]">Credential</span>
            <span className="text-[13.5px] font-bold text-[#201d1d]">{squad.outcome}</span>
          </div>
        </div>
        ...
        <Link
          href={`/learn/squads/${squad.id}`}
          className={cn(
            "flex w-full items-center justify-center rounded-full py-2.5 text-[14.5px] font-bold transition-colors",
