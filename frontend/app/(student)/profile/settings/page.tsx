          ...
          <motion.section key={group.title} variants={fadeUpItem} className="flex flex-col">
            ...
            <div className="flex flex-col overflow-hidden rounded-[20px] bg-white shadow-sm border border-black/[0.05]">
              {group.items.map((item, idx) => (
                <button
                  key={item.label}
                  className={`flex w-full items-center justify-between px-4 py-3.5 text-left transition-colors hover:bg-[#faf9f8] ${idx !== group.items.length - 1 ? "border-b border-black/[0.05]" : ""
                    }`}
                >
